...
public class CurrencyJobServiceImpl implements CurrencyJobServiceI {
    ...
    public boolean setAssetsGeneral_Job() throws Exception {
        ...
        int start = 1;
        while (true) {
            System.out.println("assetsGeneral start ======= : " + start);
    ...
    @Override
    public boolean setExchangesGeneral_Job() throws Exception {
        String url = coinConfigProperties.getExchangesGeneralJob().getUrl();
        int start = coinConfigProperties.getExchangesGeneralJob().getStart();
        int limit = coinConfigProperties.getExchangesGeneralJob().getLimit();
        String sort = coinConfigProperties.getExchangesGeneralJob().getSort();
        String sort_dir = coinConfigProperties.getExchangesGeneralJob().getSort_dir();
        String market_type = coinConfigProperties.getExchangesGeneralJob().getMarket_type();
        String convert = coinConfigProperties.getExchangesGeneralJob().getConvert();

        // 1.初始化数组，数组倒数第二个存不符合命名规范的交易所，最后一个存所有
        String indexc = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        List<List<ExchangeGeneral>> list = new ArrayList<List<ExchangeGeneral>>();
        for (int i = 0; i < indexc.length() + 2; i++) {
            List<ExchangeGeneral> temp = new ArrayList<ExchangeGeneral>();
            list.add(temp);
        }
        list.add(new ArrayList<ExchangeGeneral>()); // 存储其他名字的交易所

        JSONArray json = JSONObject
                .fromObject(setGetHasHeader(
                        url + "?start=" + start + "&limit=" + limit + "&convert=" + convert + "&market_type=" + market_type + "&sort=" + sort + "&sort_dir=" + sort_dir))
                .getJSONArray("data");

        for (int i = 0; i < json.size(); i++) {
            JSONObject item = json.getJSONObject(i);
            ExchangeGeneral exchangeGeneral = new ExchangeGeneral();
            exchangeGeneral.setEid(item.getInt("id"));
            exchangeGeneral.setName(item.getString("name"));
            exchangeGeneral.setSlug(item.getString("slug"));
            exchangeGeneral.setNum_market_pairs(item.getInt("num_market_pairs"));
            exchangeGeneral.setLast_updated(DateUtils.parseDate(item.getString("last_updated"), DateUtils.datetimeISO8601Format));
            JSONObject quote = item.getJSONObject("quote").getJSONObject(convert);

            exchangeGeneral.setVolume_24h(quote.getString("volume_24h"));
            if ("-1".equals(exchangeGeneral.getVolume_24h()))
                continue;
            exchangeGeneral.setVolume_24h_adjusted(quote.getString("volume_24h_adjusted"));
            exchangeGeneral.setVolume_7d(quote.getString("volume_7d"));
            exchangeGeneral.setVolume_30d(quote.getString("volume_30d"));
            exchangeGeneral.setPercent_change_volume_24h(quote.getString("percent_change_volume_24h"));
            exchangeGeneral.setPercent_change_volume_7d(quote.getString("percent_change_volume_7d"));
            exchangeGeneral.setPercent_change_volume_30d(quote.getString("percent_change_volume_30d"));

            int idx = indexc.indexOf(exchangeGeneral.getName().toUpperCase().charAt(0));
            if (idx < 0) {
                list.get(list.size() - 2).add(exchangeGeneral);
            } else {
                list.get(idx).add(exchangeGeneral);
            }
            list.get(list.size() - 1).add(exchangeGeneral);
        }

        for (int i = 0; i < indexc.length(); i++) {
            if (list.get(i).size() > 0)
                redisService.set(CoinConstant.CURRENCY_API + "_" + CoinConstant.EXCHNAGES_GENERAL + "_" + indexc.charAt(i), list.get(i));
        }
        redisService.set(CoinConstant.CURRENCY_API + "_" + CoinConstant.EXCHNAGES_GENERAL + "_OTHER", list.get(list.size() - 2));
        redisService.set(CoinConstant.CURRENCY_API + "_" + CoinConstant.EXCHNAGES_GENERAL + "_ALL", list.get(list.size() - 1));
        return true;
    }
    ...
    public boolean setBtcMonitorRate_Job() throws Exception {
        ...
        JSONObject quote = JSONObject.fromObject(setGetHasHeader(url.toString() + "&symbol=" + symbol + "&convert=" + convert)).getJSONObject("data").getJSONObject(symbol)
                .getJSONObject("quote");
